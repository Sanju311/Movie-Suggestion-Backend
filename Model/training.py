...
Data Pre Processing Formats Required:
    ...
    director -> categorical (bucket sort) DOESNT WORK 
             -> embeddings, too small of a training set 
        -> target encoding (average rating from user)
    producer
    writer
    cast
    
    plot -> word embedding (dense vector)
...
# Load the data
data = pd.read_csv("/Users/sanjusathiyamoorthy/Desktop/Side Projects/GitHub/Movie-Suggestion-Backend/TMDB/output/user_rated_movie_id.csv")  # Replace "data.csv" with your actual file name
...
tuned_model = XGBRegressor(**grid_search.best_params_, random_state=42)
